describe("EstudoPreliminar", function() {
 ...
 it("Verifica se consegue obter uma lista de estudo preliminar do SAE", function() {
	var result = $.ajax({
					url:  "http://localhost:2301/sae/estudo_preliminar",
					data : {},
					type: "GET",
 ...
 it("Verifica se consegue incluir, modificar e excluir um cadastro de estudo preliminar no SAE", function() {
	...
	if (result.status == 200 && objInserido.id != undefined){
			...
			// tenta incluir		   
			var result = $.ajax({
							url:  "http://localhost:2301/sae/estudo_preliminar/"+ objUpdate.id,
			...
			var result = $.ajax({
							url:  "http://localhost:2301/sae/estudo_preliminar/"+ objInserido.id,
							type: "DELETE",
							contentType: "application/x-www-form-urlencoded; charset=UTF-8",
							dataType: "json",
							crossDomain: true,
 ...
 it("Verifica se consegue pesquisar um cadastro estudo preliminar pelo id no SAE", function() {

	// faz a pesquisa
	var result = $.ajax({
					url:  "http://localhost:2301/sae/estudo_preliminar/1",
					type: "GET",
					contentType: "application/x-www-form-urlencoded; charset=UTF-8",
					dataType: "json",
					crossDomain: true,
